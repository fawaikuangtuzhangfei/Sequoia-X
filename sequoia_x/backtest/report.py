...
_CAVEATS: tuple[str, ...] = (
    ...
    "最短的可执行持有期是 T+2。",
    "幸存者偏差：退市股需先经 `--backfill-delisted` 补入，否则本地池只含"
    "当前仍在市的股票，动量类策略会被系统性高估。补入之后仍有残余——"
    "baostock 的退市名单若本身漏记，无从补起。",
    "价格为后复权序列，含未来的复权因子。",
    "「扣费后」两列按一买一卖合计 0.2% 折算（佣金+过户费+印花税+滑点的保守估计）。"
    "不含冲击成本；单笔金额大时实际更差。",
    "本地池并非全市场，横截面排名类策略（RPS）的分位数只相对本地池成立。",
    "候选池按「当日有行情」筛选。停牌股不进候选，因此本报表**不含**"
    "任何由过期 K 线判出的推荐——这与实盘路径的口径一致。",
