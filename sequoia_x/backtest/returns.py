...
- 卖出：自买入日起持有 N 个交易日，第 N 日的收盘价。N=1 即当日开盘买、收盘卖，
  这在 A 股是日内回转，**不可执行**——见 MIN_EXECUTABLE_HORIZON。
- 价格用后复权序列，已含分红送转。
...
# A 股实行 T+1 交收：当日买入的股票当日不能卖出。
#
# 本模块的 horizon=1 是"次日开盘买、**当日收盘卖**"，也就是日内回转——
# **在 A 股股票上根本执行不了**。它作为研究口径有意义（能看出信号的即时强度），
# 但绝不能照着它下单。最短的可执行持有期是 2：次日开盘买、后日收盘卖，持有一夜。
#
# 这条不是回测口径的选择，是市场规则，所以写死在这里而不是留给使用者配置。
MIN_EXECUTABLE_HORIZON: int = 2

# 默认持有期，单位是交易日。
#
# 2 和 3 是后加的，不是凑数：短线策略的超额往往在头几天内就由正转负，
# 只有 1 和 5 两个点，看不出是第 2 天就转向还是撑到第 4 天。
# 又因为 A 股 T+1 交收让 horizon=1 不可执行（见上），T+2 常常是唯一
# 既可执行又还有边际的档位——没有这一档，短线结论就无从谈起。
#
